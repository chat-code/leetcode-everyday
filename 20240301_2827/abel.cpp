...
public:
  ...
  int dfs(int pos, int dif, int x, bool bdl, bool bdr, bool num) {
    if (pos == len)
      return dif == len && x == 0 && num;

    if (!bdl && !bdr && num && dp[pos][dif][x] != -1)
      return dp[pos][dif][x];
...
int main(void) {
  ...
  Solution a;
  auto res = a.numberOfBeautifulIntegers(10, 20, 3);
  dbg(res);
  res = a.numberOfBeautifulIntegers(1, 10, 1);
  dbg(res);
  res = a.numberOfBeautifulIntegers(5, 5, 2);
  dbg(res);
  res = a.numberOfBeautifulIntegers(228, 273, 3);
  dbg(res);
